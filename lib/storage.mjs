...
import { isPublicToken, publicViewPath } from "./public-links.mjs";
import { replacementPublicUrl } from "./records.mjs";
import { getRuntimeConfig } from "./runtime.mjs";
import { assertEditableRecord, buildEditorReplacement } from "./editor-content.mjs";
import {
...
export function getLocalReadinessAccessMode({ writable = true } = {}) {
  return writable ? fsConstants.R_OK | fsConstants.W_OK : fsConstants.R_OK;
}

export async function checkStorageReadiness({ writable = true } = {}) {
  assertStorageConfigured();
  ...
  const accessMode = getLocalReadinessAccessMode({ writable });
  await Promise.all([
...
async function getStoredFile(storagePath, options = {}) {
  assertStorageConfigured();
  if (!storagePath) {
    return null;
  }
...
async function getStoredJson(storagePath, options = {}) {
  ...
  if (!options.readOnly) await ensureLocalStore();
  try {
...
function recordMutationBusy() {
  const error = new Error("Another upload mutation is still in progress");
  error.status = 409;
  error.code = "record_mutation_busy";
  return error;
}

function abandonedWriterLease() {
  const error = new Error("The upload writer lease expired; explicit recovery is required");
  error.status = 503;
  error.code = "record_writer_lease_expired";
  return error;
}

function recordIndexChanged() {
  const error = new Error("Upload records changed while this page was being read; refresh the list");
  error.status = 409;
  error.code = "record_index_changed";
  return error;
}
...
async function createStoredJsonExclusive(storagePath, value, options = {}) {
  ...
  try {
    ...
    if (handle) {
      ...
      handle = null;
      if (!options.retainOnFailure) await fs.rm(localPath, { force: true }).catch(() => {});
    }
    throw error;
  } finally {
    await handle?.close();
...
async function deleteOwnedLease(lease, options = {}) {
  if (lease.lockPath && !usesBlobStore(options)) {
    const current = await getStoredJson(lease.path, options);
  ...
  if (usesBlobStore(options)) {
    const snapshot = await getBlobJsonSnapshot(lease.path, options);
...
function combineCleanupErrors(firstError, secondError) {
  if (!firstError) return secondError;
  return new AggregateError(
    [firstError, secondError],
    "Multiple upload operation lease cleanup steps failed",
    { cause: firstError }
  );
}

function attachCleanupError(primaryError, cleanupError) {
  if (!primaryError || cleanupError === primaryError || (
  ...
  try {
    ...
    // Preserve the primary operation failure even if it cannot be annotated.
  }
}

async function cleanupOwnedLease(ownedLease, options = {}) {
  let cleanupError;
  try {
    await ownedLease.heartbeat.stop();
  } catch (error) {
    cleanupError = error;
  }
  try {
    await deleteOwnedLease(ownedLease.lease, options);
  } catch (error) {
...
async function withOwnedLeaseCleanup(ownedLease, operation, options = {}) {
  ...
  let cleanupError;
  try {
    await cleanupOwnedLease(ownedLease, options);
  } catch (error) {
    cleanupError = error;
  }

  if (operationFailed) {
    if (cleanupError) attachCleanupError(operationError, cleanupError);
...
async function verifyAcquiredLease(lease, timing, options = {}) {
  try {
    if (await readActiveMaintenance(options)) {
      throw recordIndexMaintenance();
    }
...
async function acquireWriterLease(options = {}) {
  ...
  const lease = {
    ...
    path: RECORD_INDEX_WRITER_LOCK_PATH
  };
  ...
  while (true) {
    if (await createStoredJsonExclusive(lease.path, lease, options)) {
      return verifyAcquiredLease(lease, timing, options);
    }

    const existing = await getStoredJson(lease.path, options);
    if (!existing) {
      continue;
    }
    const expiresAt = Date.parse(existing.expiresAt);
    if (!Number.isFinite(expiresAt) || expiresAt <= Date.now()) {
      throw abandonedWriterLease();
    }
    if (Date.now() >= deadline) {
      throw recordMutationBusy();
    }
...
async function acquireOrdinaryLease(kind, options = {}) {
  if (kind === "writer") return acquireWriterLease(options);
  ...
  const lease = {
    ...
    kind,
    expiresAt: leaseExpiresAt(timing.ttlMs),
    path: `${RECORD_INDEX_READER_LEASE_PREFIX}${owner}.json`
  };
...
async function withOrdinaryLease(kind, operation, options = {}) {
  ...
  return withOwnedLeaseCleanup({ heartbeat, lease }, async () => {
...
export async function withRecordMutation(operation, options = {}) {
  ...
  return withOrdinaryLease("writer", operation, options);
}
...
export async function assignPublicLink(record, { generateCode = generatePublicCode, ...options } = {}) {
  if (!writerLeaseContext.getStore()) {
    return withRecordMutation(() => assignPublicLink(record, { generateCode, ...options }), options);
  }
  assertStorageConfigured();
  if (typeof record?.id !== "string" || !/^[0-9a-fA-F-]{36}$/.test(record.id)) {
    throw new TypeError("Public links require an internal UUID");
  }
  if (record.publicCode || await getStoredJson(getRecordPath(record.id), options)) {
    throw new Error("Public links may only be assigned to fresh upload records");
  }
  const publicOrigin = getRuntimeConfig().publicOrigin;
  for (let attempt = 0; attempt < 10; attempt++) {
    const code = generateCode();
    if (typeof code !== "string" || !/^[A-Za-z0-9]{10}$/.test(code)) {
      throw new TypeError("Public code must contain exactly 10 ASCII letters or digits");
    }
    const reserved = await createStoredJsonExclusive(`public-links/${code}.json`, {
      version: 1, code, id: record.id
    }, { ...options, retainOnFailure: true });
    if (!reserved) continue;
    const assigned = { ...record, publicCode: code, publicOrigin };
    return { ...assigned, url: new URL(publicViewPath(assigned), publicOrigin).href };
  }
  const error = new Error("Public code collision retry limit exceeded");
  error.code = "public_code_collision_limit";
  error.status = 503;
  throw error;
}

export async function getRecordByPublicToken(token, options = {}) {
  assertStorageConfigured();
  ...
  try {
    let id = token;
    if (token.length === 10) {
      const mapping = await getStoredJson(`public-links/${token}.json`, readOptions);
      if (!mapping || Array.isArray(mapping) || mapping.version !== 1 || mapping.code !== token
        || typeof mapping.id !== "string" || !/^[0-9a-fA-F-]{36}$/.test(mapping.id)) return null;
      id = mapping.id;
    }
    const record = await getStoredJson(getRecordPath(id), readOptions);
    if (!record || Array.isArray(record) || record.id !== id) return null;
    if (token.length === 10 && record.publicCode !== token) return null;
    return record;
  } catch (error) {
...
export async function saveEditorReplacement(record, currentBuffer, htmlBuffer) {
  if (!writerLeaseContext.getStore()) {
    return withRecordMutation(() => saveEditorReplacement(record, currentBuffer, htmlBuffer));
  }
  assertEditableRecord(record);
  await assertRecordIndexAvailable();
  const revisionPath = `uploads/${record.id}/editor/${randomUUID()}`;
  const fileOptions = { contentType: "text/html; charset=utf-8" };
  // Never overwrite either published bytes or the existing rollback slot.
  // Retain staged/superseded files until publication is known to have succeeded.
  const previousBlob = await putStoredFile(`${revisionPath}/previous.html`, currentBuffer, fileOptions);
  const uploadBlob = await putStoredFile(`${revisionPath}/current.html`, htmlBuffer, fileOptions);
  const updatedRecord = buildEditorReplacement(record, htmlBuffer, uploadBlob, previousBlob);
  let savedRecord;
  try {
    savedRecord = await saveIndexedRecord(updatedRecord, record);
  } catch (saveError) {
    try {
      // Bypass the index gate: saveIndexedRecord may already have revoked it.
      await putStoredJson(getRecordPath(record.id), record);
    } catch (restoreError) {
      const recoveryError = new Error("Editor save failed and record restoration failed; storage recovery is required", {
        cause: saveError
      });
      recoveryError.status = 503;
      recoveryError.restoreError = restoreError;
      throw recoveryError;
    }
    throw saveError;
  }
  // Cleanup is outside compensation: a deletion failure must not undo a commit
  // or report a false save failure. Busy files are left for later recovery.
  await deleteObsoleteUploadFiles(record, savedRecord).catch(() => {});
  await deletePreviousVersion(record).catch(() => {});
  if (!shouldUseBlobStore()) {
    const obsoleteRevisions = new Set([record.blobPath, record.previousVersion?.blobPath]
      .filter(storagePath => typeof storagePath === "string"
        && storagePath.startsWith(`uploads/${record.id}/editor/`)
        && /\/editor\/[0-9a-f-]+\/(current|previous)\.html$/.test(storagePath))
      .map(storagePath => path.posix.dirname(storagePath)));
    for (const directory of obsoleteRevisions) {
      // Only remove empty revision directories, never recovery files.
      await fs.rmdir(resolveLocalStoragePath(directory)).catch(() => {});
    }
  }
  return savedRecord;
}
...
async function readRecordIndexGeneration(options = {}) {
  const marker = await getStoredJson(RECORD_INDEX_READY_PATH, options);
  if (!marker) return "empty";
  if (typeof marker.generation === "string" && marker.generation) {
    return marker.generation;
  }
  return `legacy:${String(marker.completedAt || marker.version || "v1")}`;
}

async function advanceRecordIndexGeneration(options = {}) {
  const marker = await getStoredJson(RECORD_INDEX_READY_PATH, options);
  if (!marker) return null;
  const generation = randomUUID();
  await writeRecordIndexReadyMarker({ ...marker, generation }, options);
  return generation;
}
...
async function bootstrapRecordIndexReady(options = {}) {
  ...
  await writeRecordIndexReadyMarker({ generation: randomUUID() }, options);
...
export async function saveIndexedRecord(record, previousRecord) {
  ...
  const currentRecord = await getStoredJson(getRecordPath(record.id));
  ...
  try {
    ...
    const staleIndexPaths = [previousRecord, currentRecord]
      .filter(Boolean)
      .flatMap((candidate) => {
        try {
          return [buildRecordIndexPath(candidate)];
        } catch {
          return [];
        }
      })
      .filter((indexPath) => indexPath !== indexDocument.indexPath);
    await deleteStoredFiles(staleIndexPaths);
    if (initializeReadyMarker) {
      await bootstrapRecordIndexReady();
    }
    await advanceRecordIndexGeneration();
    return savedRecord;
  } catch (error) {
...
async function listRecordsPageWithinLease(options, storageOptions) {
  ...
  const generation = await readRecordIndexGeneration(storageOptions);
  const cursorState = options.cursor
    ? decodePageCursor(options.cursor, filters)
    : { storageCursor: null, generation };
  if (cursorState.generation !== generation) throw recordIndexChanged();
  ...
  if (await readRecordIndexGeneration(storageOptions) !== generation) {
    throw recordIndexChanged();
  }
...
export async function migrateRecordIndex({
  ...
  return withOwnedLeaseCleanup(maintenance, async () => {
...
export async function getUploadContent(record, options = {}) {
  if (currentUploadKind(record) === "zip") {
    return getSiteFileContent(record, record.siteIndexPath || "index.html", options);
  }

  const upload = await getStoredFile(record.blobPath || getUploadPath(record.id), options);
  if (!upload) {
    return null;
  }
  return {
...
export async function getSiteFileContent(record, pathname, options = {}) {
  if (currentUploadKind(record) !== "zip") {
    return null;
  }
  ...
  const upload = await getStoredFile(siteFile.blobPath, options);
  if (!upload) {
    return null;
  }
  return {
...
export async function getDownloadContent(record, options = {}) {
  if (currentUploadKind(record) === "zip") {
    const upload = await getStoredFile(
      record.sourceBlobPath || getPackageSourcePath(record.id),
      options
    );
    if (!upload) {
  ...
  return getUploadContent(record, options);
}
...
export async function restorePreviousVersion(record) {
  ...
  if (previous.uploadKind === "zip") {
    ...
    restoredRecord = {
      ...
      uploadKind: "zip",
      url: replacementPublicUrl(record, "zip"),
      blobPath: undefined,
      blobUrl: undefined,
      sourceBlobPath: packageBlob.pathname,
      sourceBlobUrl: packageBlob.url,
    ...
    restoredRecord = {
      ...
      uploadedAt: new Date().toISOString(),
      uploadKind: "html",
      url: replacementPublicUrl(record, "html"),
      blobPath: uploadBlob.pathname,
      blobUrl: uploadBlob.url,
