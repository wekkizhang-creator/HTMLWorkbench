...
async function readActiveMaintenance(options = {}) {
  let maintenance;
  try {
    maintenance = await getStoredJson(RECORD_INDEX_MAINTENANCE_LOCK_PATH, options);
  } catch {
    throw recordIndexMaintenance();
  }
  if (!maintenance) return null;
  if (!isExpiredLease(maintenance)) return maintenance;
  await deleteOwnedLease({
...
function startLeaseHeartbeat(lease, timing, options = {}) {
  ...
  const renew = async () => {
    ...
    await putStoredJson(lease.path, {
      ...current,
...
async function acquireRecordIndexMaintenanceLock(options = {}) {
  ...
  while (true) {
    ...
    const current = await getStoredJson(lease.path, options);
    if (!current || isExpiredLease(current)) {
      if (current?.owner) {
        await deleteOwnedLease({ owner: current.owner, path: lease.path }, options);
      }
      continue;
    }
    throw recordIndexMaintenance();
...
async function listActiveOrdinaryLeases(options = {}) {
  ...
  for (const prefix of [RECORD_INDEX_READER_LEASE_PREFIX, RECORD_INDEX_WRITER_LEASE_PREFIX]) {
    for (const leasePath of await listAllStoragePaths(prefix, options)) {
      const lease = await getStoredJson(leasePath, options);
      if (!lease) continue;
      if (isExpiredLease(lease)) {
        await deleteOwnedLease({ owner: lease.owner, path: leasePath }, options);
      } else {
...
async function waitForOrdinaryLeases(options = {}) {
  ...
  while ((await listActiveOrdinaryLeases(options)).length > 0) {
    if (Date.now() >= deadline) throw recordIndexLeaseTimeout();
...
export async function migrateRecordIndex({
  ...
  if (dryRun) {
    await assertRecordIndexAvailable(leaseOptions);
  } else {
    maintenance = await acquireRecordIndexMaintenanceLock(leaseOptions);
  }
