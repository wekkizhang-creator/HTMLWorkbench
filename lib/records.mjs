...
import { buildPublicViewUrl } from "./runtime.mjs";
import { publicViewPath } from "./public-links.mjs";

export function replacementPublicUrl(record, kind) {
  const url = record.url || (record.publicOrigin
    ? new URL(publicViewPath(record), record.publicOrigin).href
    : publicViewPath(record));
  return `${url.replace(/\/$/, "")}${kind === "zip" ? "/" : ""}`;
}
...
export function assertUploadFile(file) {
  ...
  if (file.size > MAX_UPLOAD_BYTES) {
    const uploadError = new Error("文件超过 30 MB 限制");
