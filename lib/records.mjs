...
export function publicRecord(record) {
  return {
    ...
    tags: Array.isArray(record.tags) ? record.tags.slice(0, MAX_RECORD_TAGS) : [],
    size: record.size,
    url: record.url,
...
export function buildRecord({ fileBuffer, originalName, uploadBlob, documentType }) {
  const id = randomUUID();
...
export function buildPackageRecord({ indexBuffer, originalName, packageBlob, siteFiles, sourceSize, documentType }) {
  const id = randomUUID();
...
export function buildReplacementRecord({ record, fileBuffer, originalName, uploadBlob }) {
  const analysis = analyzeHtml(fileBuffer, originalName);
  return {
    ...record,
    originalName,
...
export function buildReplacementPackageRecord({ record, indexBuffer, originalName, packageBlob, siteFiles, sourceSize }) {
  const analysis = analyzeHtml(indexBuffer, originalName);
  return {
    ...record,
    originalName,
