...
function getPassword() {
  if (isProtectedRuntime()) {
    return requireProductionCredential(process.env, "HTML_WORKBENCH_PASSWORD");
  }
  return process.env.HTML_WORKBENCH_PASSWORD || DEFAULT_PASSWORD;
}

function getSecret() {
  if (isProtectedRuntime()) return requireStrongAuthSecret(process.env);
...
function parseValidSessionToken(token) {
  const parts = String(token || "").split(".");
  if (parts.length !== 3) return null;
  const [issuedAt, nonce, signature] = parts;
  if (!/^[0-9a-z]+$/i.test(issuedAt) || !/^[A-Za-z0-9_-]{32}$/.test(nonce)) return null;
  ...
  if (
    ...
    || !safeEqual(signature, signSessionPayload(`${issuedAt}.${nonce}`))
  ) return null;
  return {
    token,
    expiresAtMs: (issuedAtSeconds + SESSION_MAX_AGE_SECONDS) * 1000
  };
}
...
function authorizedSession(cookieHeader = "") {
  const token = parseCookies(cookieHeader).get(AUTH_COOKIE_NAME);
  return parseValidSessionToken(token);
}

export async function isAuthorizedCookie(cookieHeader = "") {
  const session = authorizedSession(cookieHeader);
  return Boolean(session) && !(await isSessionRevoked(session));
}
...
export function createCsrfToken(cookieHeader = "") {
  ...
  return createHmac("sha256", getSecret())
    .update(`html-workbench-csrf:${session.token}`)
    .digest("base64url");
}
...
export async function managementRequestFailure(request, { requireAuth = true, requireCsrf = true, allowSameOriginGet = false } = {}) {
  ...
  const sameOriginGet = allowSameOriginGet && request.method === "GET"
    && !request.headers.has("origin") && request.headers.get("sec-fetch-site") === "same-origin";
  if (!sameOriginGet && request.headers.get("origin") !== getRuntimeConfig().adminOrigin) {
    return { message: "Management requests must come from the admin origin", status: 403 };
  }
  if (requireAuth && !(await isAuthorizedRequest(request))) {
...
export async function revokeAuthorizedSession(cookieHeader = "") {
  const session = authorizedSession(cookieHeader);
  ...
  try {
    ...
    // Revocation is already durable; expiration cleanup can retry on a later logout.
  }
  return true;
}
...
export function verifyDownloadPassword(password) {
  const configured = isProtectedRuntime()
    ? requireProductionCredential(process.env, "HTML_WORKBENCH_DOWNLOAD_PASSWORD")
    : process.env.HTML_WORKBENCH_DOWNLOAD_PASSWORD || DEFAULT_PASSWORD;
