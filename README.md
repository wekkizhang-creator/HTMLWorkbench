...
当前管理入口带密码验证，默认密码是 `885688`。上传时会自动从 HTML 的 `<title>`、`meta description`、标题和正文中生成文件描述与最多 3 个标签。上传时可选择文档类型，内置 `分析报告`、`原型`、`其他`，也可以新建类型；记录页支持按标题/描述搜索，并可按文档类型和标签筛选。记录操作支持替换 HTML 文件，替换后原访问链接保持不变。
